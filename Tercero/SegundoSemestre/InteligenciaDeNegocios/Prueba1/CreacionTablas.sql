...
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[DimDOMICILIOCLIENTE]') AND type in (N'U')) DROP TABLE [dbo].[DimDOMICILIOCLIENTE]
...
CREATE TABLE DimCODIGOPOSTALSUCURSAL(
	ID_CODIGO_POTAL			    NUMERIC(3,0)    NOT NULL IDENTITY(1,1)  PRIMARY KEY,
...
CREATE TABLE DimDIRECCIONSUCURSAL(
    ...
    CONSTRAINT FK_CODIGO_POSTAL_SUCURSAL_DIRECCION_SUCURSAL FOREIGN KEY (ID_CODIGO_POSTAL_SUCURSAL) REFERENCES DimCODIGOPOSTALSUCURSAL(ID_CODIGO_POTAL),
...
CREATE TABLE DimSUCURSAL(
    ...
    DIRECCION_SUCURSAL_ID	    NUMERIC(2,0),
    CONSTRAINT FK_DIRECCION_SUCURSAL_SUCURSAL FOREIGN KEY (DIRECCION_SUCURSAL_ID) REFERENCES DimDIRECCIONSUCURSAL(ID_DIRECCION_SUCURSAL)
);
--
...
CREATE TABLE DimDOMICILIOCLIENTE(
    ID_DOMICILIO_CLIENTE	    NUMERIC(5,0)	NOT NULL IDENTITY(1,1)  PRIMARY KEY,
    NOMBRE_DOMICILIO_CLIENTE	NVARCHAR(255)
);
...
CREATE TABLE DimDIRECCIONCLIENTE(
    ...
    ID_DOMICILIO_CLIENTE	    NUMERIC(5,0),
    ...
    CONSTRAINT FK_DOMICILIO_CLIENTE_DIRECCION_CLIENTE FOREIGN KEY (ID_DOMICILIO_CLIENTE) REFERENCES DimDOMICILIOCLIENTE(ID_DOMICILIO_CLIENTE)
);
...
CREATE TABLE DimPRODUCTO(
    ...
    CATEGORIA_ID            NUMERIC(2,0),
    CONSTRAINT FK_CATEGORIA_PRODUCTO FOREIGN KEY (CATEGORIA_ID) REFERENCES DimCATEGORIA(ID_CATEGORIA)
);
CREATE TABLE DimVENTA(
    ID_VENTA            NUMERIC(7,0)	NOT NULL IDENTITY(1,1)  PRIMARY KEY,
    PRODUCTO_ID         NUMERIC(2,0),
    PRECIO_UNITARIO     NUMERIC(7,0),
    CANTIDAD            NUMERIC(7,0),
    DESCUENTO           NUMERIC(4,2),
    PRECIO_TOTAL        NUMERIC(8,0),
    CONSTRAINT FK_PRODUCTO_VENTA FOREIGN KEY (PRODUCTO_ID) REFERENCES DimPRODUCTO(ID_PRODUCTO)
);
--
CREATE TABLE DimHORA(
    ID_HORA         NUMERIC(2,0)	NOT NULL IDENTITY(1,1)  PRIMARY KEY,
    HORA            NUMERIC(2,0),
    MINUTO          NUMERIC(2,0)
);
CREATE TABLE DimDIA(
    ID_DIA          NUMERIC(2,0)	NOT NULL IDENTITY(1,1)  PRIMARY KEY,
    DIA             NUMERIC(2,0)
);
CREATE TABLE DimMES(
    ID_MES          NUMERIC(2,0)	NOT NULL IDENTITY(1,1)  PRIMARY KEY,
    MES             NUMERIC(2,0)
);
CREATE TABLE DimANNO(
    ID_ANNO         NUMERIC(4,0)	NOT NULL IDENTITY(1,1)  PRIMARY KEY,
    ANNO            NUMERIC(4,0)
)
