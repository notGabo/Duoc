...
eesy.define(['jquery-private', 'utils', 'helpitem-loader', 'json!settings-supportcenter'], function ($, utils, helpItemLoader, settings) {
    ...
    utils.onClickOrSelectKey('.__explain-downvoting .form__submit_Sup', function (e, element) {
        handleSubmitVoteSupport(element, false);
    });
    utils.onClickOrSelectKey('.__explain-upvoting .form__submit_Sup', function (e, element) {
        handleSubmitVoteSupport(element, true);
    });
    function handleSubmitVoteSupport(element, isUpVote) {
        var reason = $(".layout__main " + (isUpVote ? ".__explain-upvoting" : ".__explain-downvoting") + " .quick-survey__textarea").val();
        sendVoteSupport(reason, element, isUpVote);
    }
        ...
        if (settings.SUPPORTCENTER.VOTINGOPTIONS.ENABLED) {
            $.getJSON(var_dashboard_url + "/rest/helpitems/options/voting?type=" + direction + "&sessionkey=" + var_key, function (votingoptions) {
                ...
                votingoptions.forEach(function (votingOption) {
                    var checkboxElement = '<input type="checkbox"  aria-label="' + votingOption.caption + '" data-id="' + votingOption.id + '" data-text="' + votingOption.caption + '">' + votingOption.caption + '<br>';
    ...
    utils.onClickOrSelectKey('.__explain-downvoting .form__submit', function (e, element) {
        handleSubmitVoteMessage(element, false);
    });
    utils.onClickOrSelectKey('.__explain-upvoting .form__submit', function (e, element) {
        handleSubmitVoteMessage(element, true);
    });
    function handleSubmitVoteMessage(element, isUpVote) {
        hid = $(element).closest(".quick-survey-section").find(".isSelected").data("helpitemId");
        var reason = $(element).closest(".quick-survey").find(".quick-survey__textarea").val();
        sendVoteMessage(reason, element, isUpVote);
    }
    function sendVoteMessage(reason, element, isUp) {
        var voteOptionsById = {};
