...
eesy.define(['jquery-private', 'sessionInfo', 'engine-state', 'json!monitor-data'], function ($, sessionInfo, engineState, contextmonitors) {
    ...
    function handleMonitor(monitorWithCourseid) {
        ...
        $.ajax({
            url: sessionInfo.dashboardUrl() + "/rest/public/monitors/" + monitorWithCourseid.monitorId + "/triggered" +
                ("?sessionkey=" + sessionInfo.sessionKey() + "&courseid=" + monitorWithCourseid.courseId + "&_=" + new Date().getTime()),
            type: 'PUT',
