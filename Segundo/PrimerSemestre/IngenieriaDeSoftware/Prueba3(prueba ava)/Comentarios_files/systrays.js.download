...
eesy.define(['jquery-private', 'utils', 'mustachejs',
    ...
    function hide(helpItemId) {
        ...
        if (helpItemId) {
            eesyTimers.stop("helpitem" + helpItemId);
        }
        else {
            eesyTimers.stopAll();
        }
    ...
    function show(helpItem) {
        ...
        if (utils.cookieExists("eesysystrayhidden_" + helpItem.id + "_" + var_key))
            return;
        $("body").append(Mustache.to_html(eesyTemplates.systray, presentationHelper.helpItemModel(helpItem), eesyTemplates));
    ...
    function positionSystray(helpItem) {
        ...
        if ($('#systraycontainer').data("helpitemid") != helpItem.id)
            return;
        var survey = $("#systraycontainer[data-helpitemid=\"" + helpItem.id + "\"] .quick-survey-section");
        var surveyheight = survey.length ? survey.height() + 30 : 0;
