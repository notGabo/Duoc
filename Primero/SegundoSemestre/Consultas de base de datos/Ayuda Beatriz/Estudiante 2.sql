...
upper(MATERNO)||' está de cumpleaños el '||TO_CHAR(fecnac,'DD')||' de '||TRIM(INITCAP(TO_CHAR(fecnac,'MONTH')))||'. Cumple '||
trunc(MONTHS_BETWEEN(ADD_MONTHS(SYSDATE,-1),FECNAC)/12)||' años.' as "LISTADO DE CUMPLEAÑOS"
FROM EMPLEADO
WHERE FECNAC IS NOT NULL
ORDER BY TO_NUMBER(TO_CHAR(FECNAC,'DD')), PATERNO;
--CASO 5
--Primera opcion
SELECT 
...
SELECT 
NOMBRE||' '||PATERNO||' '||MATERNO AS "NOMBRE COMPLETO",
TO_CHAR(FECING,'YYYY')||SUBSTR(LOWER(NOMBRE),1,1)||SUBSTR(LOWER(MATERNO),-2)||'@corredorah.cl' AS "CORREO EMPLEADO",
CASE 
    WHEN SUBSTR(PATERNO,1,1) BETWEEN 'A' AND 'E' THEN 'Lunes 24 de Septiembre'
    WHEN SUBSTR(PATERNO,1,1) BETWEEN 'F' AND 'J' THEN 'Lunes 1 de Octubre'  
    WHEN SUBSTR(PATERNO,1,1) BETWEEN 'K' AND 'N' THEN 'Lunes 8 de Octubre'
    WHEN SUBSTR(PATERNO,1,1) BETWEEN 'O' AND 'S' THEN 'Lunes 22 de Octubre'
    ELSE 'Miercoles 24 de Octubre'
END || ' del 2018' AS "FECHA DE EVALUACION" 
FROM EMPLEADO
...
SELECT 
NUMPROPIEDAD AS "NRO PROPIEDAD",
RUTPRO AS "RUT PROPIEDAD",
DIRECCION AS "DIRECCIÓN",
RTRIM(TO_CHAR(RENTA,'$9G999G999')) AS "VALOR DEL ARRIENDO",
CASE 
    WHEN RENTA >=1000000 THEN TO_CHAR(ROUND((TO_NUMBER(REGEXP_SUBSTR(RENTA,'(\d)(\d)'))/100)*RENTA),'$9G999G999')
    WHEN RENTA <1000000 THEN TO_CHAR(ROUND((TO_NUMBER(REGEXP_SUBSTR(RENTA,'(\d)'))/100)*RENTA),'$9G999G999')
END AS COMPENSACION
FROM PROPIEDAD
ORDER BY 1 ASC;
