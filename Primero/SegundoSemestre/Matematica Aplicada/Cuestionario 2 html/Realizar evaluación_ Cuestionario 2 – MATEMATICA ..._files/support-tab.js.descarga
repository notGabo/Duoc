...
eesy.define([
    ...
    $(document).on('click', '#eesy-tab-inner', function () {
        if (!$('#eesy-tab-inner').hasClass('eesy_draggable') && $("#buildModeBar").length == 0) {
            onClickSupportTab();
        }
    });
    ...
    function dragButton(e) {
        ...
        var topOffset = posY - $('.eesy_draggable').outerHeight() / 2;
        var top = var_tab_version === 3 ? topOffset : Math.max(Math.min(topOffset, maxY), minY);
        $('.eesy_draggable').offset({ top: top });
        e.preventDefault();
    }
    function stopMove() {
